...
.. note::
    In v6.0, the channel map view only supports raster rendering of an image cube. Contour rendering, vector field rendering, and catalog overlay are not supported. In a future release, we will support the other rendering modes including the matching feature.
...
.. note::
    In v6.0, the region of interest and annotation objects are implicitly applied to the entire spectral axis of an image cube. Having the ability to specify a range of the spectral axis and a set of Stokes will be available in a future release of CARTA. This is also know as 3D or 4D region of interest.
